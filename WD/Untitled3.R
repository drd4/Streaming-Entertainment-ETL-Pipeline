...
cvBoostrep = data(price ~ price_mean_c +  bedrooms + cleaning_fee + room_type + minimum_nights 
                 + accommodates + property_type + bathrooms + beds + neighbourhood_cleansed + host_is_superhost
                 + availability_30 + review_scores_rating + availability_60 + reviews_per_month 
                 + availability_90 + Airconditioning + Dryer + Elevator + Family_and_kidfriendly + Freestreetparking
                 ...
                 ,data = data, method = 'gbm', trControl = trControl_boost6, tuneGrid = tuneGrid_boost6)

cvBoostrep

### start here*********
## to save time, I only use 2,000 trees here, the best model used 30,000 trees
boostCVrep = gbm(price ~ price_mean_c +  + bedrooms + cleaning_fee + room_type + minimum_nights 
                 + accommodates + property_type + bathrooms + beds + neighbourhood_cleansed + host_is_superhost
                 + availability_30 + review_scores_rating + availability_60 + reviews_per_month 
                 + availability_90 + Airconditioning + Dryer + Elevator + Family_and_kidfriendly + Freestreetparking
                 + review_scores_cleanliness + Hairdryer + Iron + Oven + Refrigerator + Shampoo + number_of_reviews 
...
pred_boostscorerep = predict(boostCVrep, n.trees = 200, newdata = scoring)

submissionFilerep = data.frame(id = scoring$id, price = pred_boostscorerep)
write.csv(submissionFilerep, 'boostrep.csv',row.names = F)
