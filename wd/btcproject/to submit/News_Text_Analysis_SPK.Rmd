...
## Load Data
data <- read.csv('news_title_data_2.csv', stringsAsFactors = FALSE)
...
news_title_char <- data %>%
                    select(news_title)%>%
                    mutate(characters = nchar(news_title),
                           words = str_count(news_title, pattern='\\S+'),
                           sentences = sapply(strsplit(data$news_title, "[.?!]+"), length))

## Mean
mean <- news_title_char %>% summarize_at(c('characters','words','sentences'),.funs = mean,na.rm=T)

## Variance
variance <- news_title_char %>% summarize_at(c('characters','words','sentences'),.funs = var,na.rm=T)

## Pearson correlation
correlation <- cor(news_title_char[,c('characters','words','sentences')])

## Rank correlation
cor(news_title_char[,c('characters','words','sentences')], method='spearman')

summary_stats_df <- data.frame(mean = c(mean$characters, mean$words, mean$sentences),
                               variance = c(variance$characters, variance$words, variance$sentences))

rownames(summary_stats_df) <- c('characters','words','sentences')
correlation
summary_stats_df
```
...
news_title_data <- cbind(btc_pc_avg = data$btc_pc_avg, xdtm)
news_title_data_tfidf <- cbind(btc_pc_avg = data$btc_pc_avg, xdtm_tfidf)
...
```{r predictions}
## TDF
lasso_tdf <- cv.glmnet(as.matrix(train_tdf[,-1]), train_tdf$btc_pc_avg, family="gaussian", alpha=1)

## TF-IDF
lasso_tfidf <- cv.glmnet(as.matrix(train_tfidf[,-1]), train_tfidf$btc_pc_avg, family="gaussian", alpha=1)

## TDF vs. TF-IDF RMSE
# TDF
rmse_lasso_tdf <- sqrt(mean((predict(lasso_tdf, as.matrix(test_tdf)[,-1]) - test_tdf$btc_pc_avg)^2))
# TF-IDF
rmse_lasso_tfidf <-sqrt(mean((predict(lasso_tfidf, as.matrix(test_tfidf)[,-1]) - test_tfidf$btc_pc_avg)^2))
```
...
which.min(tune_nrounds_xgb$evaluation_log$test_rmse_mean) #78
...
rmse_xgboost_tdf <-sqrt(mean((xgb_pred_tdf - test_tdf$btc_pc_avg)^2)); rmse_xgboost_tdf
...
rmse_xgboost_tfidf <-sqrt(mean((xgb_pred_tfidf - test_tfidf$btc_pc_avg)^2)); rmse_xgboost_tfidf 
```
...
train_components_tdf = data.frame(cbind(btc_pc_avg = train_tdf$btc_pc_avg, train_tdf_pca$x[,1:58]))

# Transforms original data to PCA space
test_tdf_pca = predict(train_tdf_pca, newdata=testPredictors_tdf)
test_components_tdf = data.frame(cbind(btc_pc_avg = test_tdf$btc_pc_avg, test_tdf_pca [,1:58]))
...
train_components_tfidf = data.frame(cbind(btc_pc_avg = train_tfidf$btc_pc_avg, train_tfidf_pca$x[,1:58]))

# Transforms original data to PCA space
test_tfidf_pca = predict(train_tfidf_pca, newdata=testPredictors_tfidf)
test_components_tfidf = data.frame(cbind(btc_pc_avg = test_tfidf$btc_pc_avg, test_tfidf_pca [,1:58]))
```